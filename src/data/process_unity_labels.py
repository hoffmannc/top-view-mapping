...
def reduce_labels(labels):
    ...
    reduced[0, :, :] = reduced[1:4, :, :].sum(dim=0) == 0  # Drivable area
    reduced[4, :, :] = labels[14, :, :]  # Mask
    return reduced


target_path = "/Users/colinhoffmann/Desktop/top-view-mapping/data/nuscenes/targets/"
input_path = "/Users/colinhoffmann/Desktop/nuscenes/map-labels-v1.2/"
...
for i, filename in enumerate(os.listdir(input_path)):
    ...
    final_labels = encode_labels(labels)
    output_path = target_path + filename
    Image.fromarray(final_labels.numpy().astype(np.int32), mode="I").save(output_path)
