...
class PositionalEncoding_00(nn.Module):
    r"""Inject some information about the relative or absolute position of the tokens
        in the sequence. The positional encodings have the same dimension as
        the embeddings, so that the two can be summed. Here, we use sine and cosine
        functions of different frequencies.
    ...
    def __init__(self, d_model, max_len=5000):
        super(PositionalEncoding_00, self).__init__()
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x):
        ...
        dim_t = self.temperature ** (2 * (i // 2) / self.num_pos_feats)
...
class PositionalEncoding2DwDropout(nn.Module):
    ...
    def forward(self, x, height, width):
        pe = torch.zeros(self.d_model, height, width, device=x.device)
